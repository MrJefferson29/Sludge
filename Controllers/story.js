...
const addStory = asyncErrorWrapper(async  (req,res,next)=> {
    ...
    const {title, content, address, status, slider, insurrance} = req.body 
    ...
    try {
        ...
        return res.status(200).json({
            success :true ,
            message : "Good Job, story added successfully ",
            data: newStory
        })
    }
...
const editStory  =asyncErrorWrapper(async(req,res,next)=>{
    const {slug } = req.params ; 
    const {title ,content ,image ,previousImage, status, timeLeft, address, lon, lat } = req.body;
